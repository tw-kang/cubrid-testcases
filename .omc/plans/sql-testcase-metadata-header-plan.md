...
- v5 — Loop iter 5 (critic synthesis). PR3 reference migration (`_13_issues/_24_1h/cbrd_25054.sql`, `_26_1h/cbrd_26999_metadata_sample.sql`) deferred to a follow-up PR; `_36_guava` 12-file migration shipped instead per user priority redirect. SQL-META001 split into META001 (header/key absence), META009 (FS guards: symlink/non-regular/oversize), META010 (empty/BOM-only). Inline-directive inside header block now terminates parsing (META_LINE_LOOSE no longer matches `--@<word>`).
